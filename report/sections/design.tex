...
\textbf{Registered users} has all anonymous permissions and can additionally create polls, comment on polls, 
...
support nested replies through a recursive \texttt{parent} relationship. A \texttt{Comment} may have 
nested child (comments via its \texttt{parent} field). Comments are deleted if their parent or poll is 
removed.
...
specific configuration issues. Docker Compose allows the entire system to be started with a single command, which simplifies 
development, testing and onboarding.
