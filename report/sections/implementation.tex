...
while creating polls and comments requires authentication. Below is a brief outline of the security rules from our configuration:
...
The modular design makes it straightforward to add new poll types, adding friends, or stronger authentication, 
and a mobile client could reuse the existing REST and WebSocket APIs. Planned improvements include 
hardened production security and a durable database. Because of time constraints and limited hardware, 
we could not run large tests ourselves. 
Instead, we use published large-scale results to confirm the observations we see and to give 
a broader context for performance we cannot test yet but want to explore in the future.
