...
how events reach clients, and what can be extended. Listings are trimmed to only illustrate the key mechanisms.
...
docker compose up -d
\end{lstlisting}
The UI is served at \texttt{http://localhost:5173}. A single WebSocket complements REST for real-time updates.
...
\end{lstlisting}
Each successful write broadcasts a lightweight WebSocket hint so clients refresh selectively:
\begin{lstlisting}[language=Java]
...
Producer and consumer are thin wrappers:
\begin{lstlisting}[language=Java]
// Producer
...
\end{lstlisting}
Topics are created with one partition (replication factor three) for ordered, fault‑tolerant updates.
...
limited timeconstraints we were not able to conduct them.
