...
\paragraph{Event: }
An event is the most basic unit of data.
...
\paragraph{Topic: }
A category where events are stored.
\textbf{FeedApp} employs a dynamic strategy: the \\\texttt{PollTopicManager} creates a unique topic for every poll (e.g., \texttt{poll.voteChange.10}) rather than using a single monolithic topic.
KRaft mode allows this to scale to millions of topics\cite{kafka_topics}.

\paragraph{Producer: }
A producer is a client application that writes events to a Kafka topic.
...
as \textbf{poll-app} in its \texttt{application.properties} file.
...
(\texttt{KAFKA\_CONTROLLER\_QUORUM\_VOTERS\@: '1@kafka-1:9093,\dots'}).
