...
of an application can write to and read from simultaneously. The \textbf{FeedApp}
...
In the \textbf{FeedApp}, an event is a
\texttt{Map<String, Object>} that gets serialized into JSON, for example, when
a poll is created, the \textit{PollEventListener} creates an event with the
\textit{pollId}, \textit{question}, and \textit{eventType}.
The consumer listens for vote change events, which it receives
as a \textit{Map} containing the \textit{pollId} and \textit{optionOrder}.
...
The \textbf{FeedApp} uses a dynamic topic strategy.
Instead of one giant \textit{votes} topic, the \textit{PollTopicManager}
creates a unique topic for every single poll.
For example, for a poll with ID \textit{10}, the topic name would be
\textit{poll.voteChange.10}.

\subsubsection*{Producer:}
A producer is a client application that writes
events to a Kafka topic.
The \textit{ProducerService} in the FeedApp project is a classic producer,
using Spring's \textit{KafkaTemplate} to send events.
This service is used by the \textit{PollEventListener} to send a message
after a new poll is successfully saved to the database.
With the 3-node cluster, the producer (our backend) is given a list
of all three brokers (\textit{kafka-1:29092,kafka-2:29092,kafka-3:29092})
...
The \textit{ConsumerService} acts as the consumer
.
It uses a powerful feature to match the dynamic topic strategy: it listens to a
\textit{topicPattern} (\textit{poll.voteChange.*}) instead of a fixed topic
...
The \textbf{FeedApp} defines the consumer group ID
as \textbf{poll-app} in its \textit{application.properties} file.
...
The \textbf{FeedApp} project makes a specific design
choice. When the \textit{PollTopicManager} creates a new topic, it is
...
by calling \textit{new NewTopic} with the parameters
\textit{(topicName, 1, (short) 3)}.
...
The provided \textit{docker-compose.yml}
file defines three distinct broker services: \textit{kafka-1}, \textit{kafka-2},
and \textit{kafka-3}. Each of these nodes
is configured with the \textit{broker} role.
...
The three brokers (\textit{kafka-1},
\textit{kafka-2}, \textit{kafka-3}) form the cluster.
This is where the concept of a replication factor of 3 becomes reality.
When \textit{PollTopicManager} creates \textit{poll.voteChange.10} with 3
...
(\textit{KAFKA\_CONTROLLER\_QUORUM\_VOTERS\@: '1@kafka-1:9093,\dots'}).
...
consumerPerSubscription: 1
producersPerTopic: 1
producerRate: 100000 # This will vary, 1000,10000,100000
consumerBacklogSizeGB: 0
testDurationMinutes: 3
\end{lstlisting}
